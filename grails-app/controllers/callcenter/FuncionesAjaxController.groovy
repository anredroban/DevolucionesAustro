...
class FuncionesAjaxController {
    ...
    def getCiudades() {
        ...
        if (params.id) {
            ...
            def ciudades = Agencia.executeQuery("from Agencia where provincia = :provincia and isActive = true order by nombre", [provincia: provincia])
    ...
    def getParroquiasAgencias() {
        if (params.id) {
            Ciudad canton = Ciudad.findById(params.id)
            def parroquias = ParroquiaAgencia.executeQuery("from ParroquiaAgencia where ciudad = :canton order by nombre", [canton: canton])
